...
def generate_logstash_config(log_sample, desired_output, input_config, output_config, log_schema, filter_rules, log_type_name, existing_code=None, error_message=None):
    ...
    if error_message:
        prompt = f"""The following Logstash configuration failed. Please fix the logic inside the 'filter' block based on the error message, strictly following the rules below.
        FILTER RULES:{filter_rules}
        Here is Log Schema of {log_type_name} (for context): {log_schema}
        ...
        prompt = f"""Write a complete Logstash configuration.
        ...
        Here is Log Schema of {log_type_name}: --- {log_schema} ---
    ...
    print("--- 🤖 Đang gửi yêu cầu (với quy tắc động từ .env) đến AI... ---")
    try:
...
def main():
    ...
    log_type_name = os.getenv("LOG_TYPE_NAME", "Unknown")
    ...
    required_vars = {
        ...
        "LOG_INPUT_FILE_PATH": log_input_path, "LOGSTASH_CONFIG_PATH": final_config_path,
...
if __name__ == "__main__":
    if not os.getenv('GOOGLE_API_KEY'):
        print("LỖI: Biến GOOGLE_API_KEY không tìm thấy. Vui lòng kiểm tra file .env của bạn.")
    else:
        main()
...
def main():
    ...
    log_type_to_process = sys.argv[1]

    if not initialize_env(log_type_to_process):
        return

    # --- LẤY CÁC BIẾN CẤU HÌNH TỪ .env ĐÃ ĐƯỢỢC TẢI ---
    ...
    # Xây dựng cấu hình input/output từ các biến
    # Kiểm tra nếu người dùng muốn dùng syslog input
    if log_input_path:
         input_config = f"""file {{ path => "{log_input_path}" start_position => "beginning" }}"""
    else:
