...
OLLAMA_MODEL_NAME = os.getenv("llama3.2:latest ", "llama3") 
...
async def parse_log_with_ai(session: aiohttp.ClientSession, raw_log: str, headers_str: str) -> dict:
    """Sử dụng model Ollama local để parse một dòng log CSV thô."""
    ...
    full_prompt = LOG_PARSER_PROMPT.format(header_fields=headers_str, raw_log=raw_log)
    
    payload = {
        "model": OLLAMA_MODEL_NAME,
        "prompt": full_prompt,
        "stream": False,
        "format": "json"  # Yêu cầu Ollama trả về output dạng JSON hợp lệ
    }

    try:
        logging.info(f"Đang gửi log đến Ollama ({OLLAMA_MODEL_NAME}) để parse: {raw_log[:120]}...")
        async with session.post(OLLAMA_API_URL, json=payload) as response:
            if response.status == 200:
                data = await response.json()
                # Phản hồi của Ollama là một JSON, trong đó có key "response" chứa chuỗi JSON kết quả
                json_string_from_model = data.get("response")
                if json_string_from_model:
                    return json.loads(json_string_from_model)
                else:
                    logging.warning("Ollama không trả về nội dung trong key 'response'.")
                    return None
            else:
                logging.error(f"Lỗi từ API Ollama: {response.status} - {await response.text()}")
                return None
    except aiohttp.ClientConnectorError as e:
        logging.error(f"Lỗi kết nối đến Ollama tại {OLLAMA_API_URL}. Hãy chắc chắn Ollama đang chạy. Lỗi: {e}")
        return None
    except Exception as e:
        logging.error(f"Lỗi không xác định khi gọi API Ollama hoặc parse JSON: {e}")
        return None
...
async def send_to_elk(es_client: AsyncElasticsearch, document: dict):
    ...
    index_name = f"logs-local-{datetime.now(timezone.utc).strftime('%Y.%m.%d')}"
...
async def main():
    ...
    async with aiohttp.ClientSession() as session: # Tạo một session duy nhất để tái sử dụng
        try:
            async for msg in consumer:
                raw_log_string = msg.value.decode('utf-8')
                # Truyền session vào hàm parse
                parsed_log = await parse_log_with_ai(session, raw_log_string, headers_for_prompt)
